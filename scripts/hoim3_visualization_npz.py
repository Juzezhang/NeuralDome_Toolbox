...
from models.body_models_easymocap.smplx import SMPLModel
...
parser.add_argument('--seq_name', type=str, default="office_data02")
parser.add_argument('--resolution', type=int, default=720)
parser.add_argument('--step', type=int, default=60)
parser.add_argument('--output_path', type=str, default="/nas/nas_38/AI-being/libin/M3_repair")
...
seq_step = args.step
body_model_easymocap = SMPLModel(model_path='/data/NeuralDome_Toolbox/models/model_files/smpl/SMPL_NEUTRAL.pkl', device=device)

model_path = '/data/NeuralDome_Toolbox/models/model_files/'
...
for date in calibration_dates:
    if args.seq_name in dataset_info[date]:
        ...
        calibration_path = join('/nas/nas_38/AI-being/libin/M3_repair/npz_id_new/new_calibration', date, 'calibration.json')
...
output_dir = join(args.output_path, 'vis_id', args.seq_name, str(view_index))
...
smpl_param_path = join(args.output_path, 'npz_id_new', f"{args.seq_name}_human.npz")
smpl_data = np.load(smpl_param_path,allow_pickle=True)['smpl_params']

object_path = join(args.output_path, 'npz_id_new', f"{args.seq_name}_object.npz")
object_rotation_refinement_all = np.load(object_path,allow_pickle=True)['object_params']
...
for frame_count in tqdm(range(start_frame, end_frame), desc="Processing frames"):
    ...
    smpl_params_list = []
    for smpl_param in smpl_data[frame_count-start_frame]:
    ...
    for smpl_param in smpl_params_list:
        # smpl_param = body_model_easymocap.convert_to_standard_smpl_Th(smpl_param)
        vertices_mesh = body_model(
            betas=torch.tensor(smpl_param['shapes']).float(),
            body_pose=torch.tensor(smpl_param['poses']).float(),
            global_orient=torch.tensor(smpl_param['Rh']).float(),
            transl=torch.tensor(smpl_param['Th']).float(),
        )['vertices']
        out_mesh_list.append(vertices_mesh)


    human_mesh_list = []
    for out_mesh in out_mesh_list:
        # Create a trimesh object for the human mesh
        human_mesh = trimesh.Trimesh(vertices=out_mesh[0].detach().cpu().numpy(), faces=body_model.faces)
    ...
    for object_idx, object_name in enumerate(object_names):
        ...
        object_rotation_refinement = object_rotation_refinement_all[frame_count-start_frame][object_name]
