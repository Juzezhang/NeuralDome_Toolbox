...
class SimpleTransform3DSMPL(object):
    ...
    def __call__(self, src, label):
        if self.two_d:
            ...
            # if 'distCoef' in label.keys():
            #     distCoef = label['distCoef'].copy()
            # else:
            #     distCoef = None
            ...
            # if imgwidth == src.shape[1] and imght == src.shape[0]:
            #     aaa = 1
            # else:
            #     aaa = 1
...
class SimpleTransform_lidarhuman26m(object):
    """Generation of cropped input person, pose coords, smpl parameters.
    ...
    def __call__(self, src, label):
        ...
        if self.two_d:
            output = {
                ...
                'depth_factor': torch.from_numpy(depth_factor).float(),
                'bbox': torch.Tensor(bbox),
                'image_offset': torch.from_numpy(label['image_offset']).float()
            }
